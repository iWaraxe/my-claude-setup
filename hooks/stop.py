...
def get_intelligent_completion_message(input_data: dict):
    """
    Generate intelligent completion message based on session transcript analysis.
    Falls back to LLM services or random messages if analysis fails.
    ...
    try:
        ...
        # Fallback to original LLM-based approach on any error
        pass
    
    # Original LLM completion message logic as fallback
...
def announce_completion(input_data: dict):
    """Announce completion using the best available TTS service with intelligent summaries."""
    try:
        tts_script = get_tts_script_path()
        if not tts_script:
            return  # No TTS scripts available
        
        # Get intelligent completion message (with fallback to LLM/random)
        completion_message = get_intelligent_completion_message(input_data)
...
def main():
    try:
        ...
        # Announce completion via TTS with intelligent summary
